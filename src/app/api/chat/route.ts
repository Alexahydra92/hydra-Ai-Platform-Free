...
// ─── Default AI using z-ai-web-dev-sdk (no API key needed) ───────────
async function handleDefaultAI(messages: { role: string; content: string }[]) {
  try {
    const zai = await ZAI.create();

    // Filter and validate messages for Z.ai SDK
    ...
    const stream = new ReadableStream({
      start(controller) {
        // Send the full content as a single chunk in SSE format
        const chunk = {
          choices: [
            {
              delta: { content },
...
export async function POST(req: NextRequest) {
  try {
    ...
    // If useDefault is true OR no API key provided → use Z.ai default
    if (useDefault || !apiKey) {
      return await handleDefaultAI(messages);
    }
